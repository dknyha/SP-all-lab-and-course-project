...
mas2 dd 1.4, 2.1, 3.8, 4.6, 5.4, 6.12,0
mas1 dd -1.34,-5.0,-3.54,1.5,-5.8,-6.53,0
...
titl1 db "masm64. Паралельне порівняння за допомогою SSE-команд",0; название окошка
buf1 dq 0,0 
...
jz m7
movups XMM0,[rdi] ; mas2
jmp m8
m7: movups XMM0,[rsi] ; mas1 ;
m8:

unpckhpd xmm4,xmm0 ; розпакування ст. ч. xmm0 у ст. ч. xmm4 та зсув мол. ч. xmm4
unpckhpd xmm4,xmm5 ; переміщення ст. частини xmm4 у мол. ч. xmm4
unpcklpd xmm5,xmm0 ; розпакування мол. ч. xmm0 у ст. ч. xmm5 та зсув мол. ч. xmm5
unpckhpd xmm5,xmm6 ; переміщення ст. частини xmm5 у мол. частина xmm5
addps xmm4,xmm5 ; сума xmm4 та xmm5
...
cmp r10,0 	; проверка знаковых битов
jz m9
movss xmm3,dword ptr [rdi] ; mas2
jmp m0
m9: movss xmm3,dword ptr [rsi] ; mas1 ;
m0:
addps xmm4,xmm3 ; сума xmm4 та xmm3
...
unpckhpd xmm6,xmm8
unpckhpd xmm7,xmm8
addps xmm6,xmm7 ; сума xmm6 та xmm7
m5:
cvtps2dq xmm2,xmm6
